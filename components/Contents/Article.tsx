import { Editor, EditorContent } from "@tiptap/react";
import React, { useState } from "react";
import EditorMenu from "../TipTap/EditorMenu";
import { RocketLaunchIcon } from "@heroicons/react/24/solid";

type Props = {
  contentError: string;
  editor: Editor | null;
  isEditable: boolean;
  setContent: (content: string) => void;
  title: string;
  titleError: string;
};

function Article({
  contentError,
  editor,
  isEditable,
  setContent,
  title,
}: Props) {
  const [role, setRole] = useState<string>("I am a helpful Assistant");

  // const handleFetchAIContent = async () => {
  //   editor
  //     ?.chain()
  //     .focus()
  //     .setContent("GENERATING AI CONTENT... Please Wait...")
  //     .run();

  //   const response = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/openai`, {
  //     method: "POST",
  //     headers: {
  //       "Content-Type": "application/json",
  //     },
  //     body: JSON.stringify({
  //       title: title,
  //       role: role,
  //     }),
  //   });

  //   const data = await response.json();

  //   editor?.chain().focus().setContent(data.content).run();

  //   setContent(data.content);
  // };

  if (!editor) {
    return null;
  }

  return (
    <article className="text-wh-500 leading-8">
      {/* {isEditable && (
        <div className="border-2 rounded-md bg-wh-50 p-3 mb-3">
          <h4 className="m-0 p-0">Generate AI Content</h4>
          <p className="my-1 p-0 text-xs">What Type of Writer do you want?</p>
          <div className="flex gap-5 justify-between">
            <input
              className="border-2 rounded-md bg-wh-50 p-3 mb-3"
              placeholder="Role"
              onChange={(e) => setRole(e.target.value)}
              value={role}
            />
            <button type="button">
              <RocketLaunchIcon
                onClick={handleFetchAIContent}
                className="h-8 w-8 text-accent-red hover:text-wh-300"
              />
            </button>
          </div>
        </div>
      )} */}

      <div
        className={`${
          isEditable ? "border-2 rounded-md bg-wh-50 p-3" : "w-full max-w-full "
        }`}
      >
        {isEditable && (
          <>
            <EditorMenu editor={editor} />
            <hr className="border-1 mt-2 mb-5" />
          </>
        )}
        <EditorContent editor={editor} />
      </div>
      {contentError && <p className="mt-1 text-wh-900">{contentError}</p>}
    </article>
  );
}

export default Article;
